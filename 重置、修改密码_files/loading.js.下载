define('components/loading/loading.tpl',['hdbr'], function(Handlebars) {
			return Handlebars.template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1, helper, alias1=depth0 != null ? depth0 : (container.nullContext || {}), alias2=helpers.helperMissing, alias3="function";

  return "<div class=\"sn-ui-loading "
...
define('text!components/loading/loadingLine.tpl',[],function () { return '<div class="sn-ui-loading-progress {{ className }}">\r\n  <div class="sn-ui-loading-progress-content"></div>\r\n</div>';});
...
define('loading',[
  ...
  var objClass = function(options) {
    ...
    this.options = {
      ...
      content: ''  //自定义显示信息。设置后text和icon配置就失效了。
    },
    ...
    function parentPosition() {
      var elPosition = $.trim(this.$el.css('position'));
      elPosition=elPosition.toLocaleLowerCase();
...
//      var template = hdb.compile(lineTmpl);
        _$uiBox      = $(lineTemplate(this.options));
        _$uiContent  = _$uiBox.find('.sn-ui-loading-progress-content');
        $('body').append(_$uiBox);
        return;
      }
...
//    var template = hdb.compile(tpl);
      ...
      _$uiContent  = _$uiBox.find('.ui-loading-content');
      _$uiBackdrop = _$uiBox.find('.ui-loading-backdrop');
    ...
    function loadingLine() {
      // 页面中只允许存在一个对象
      if($('body > .sn-ui-loading-progress').length >= 1) {
        $('body > .sn-ui-loading-progress').remove();
      }
